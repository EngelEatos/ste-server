...
func testLanguageToManyNovels(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&b.LanguageID, a.ID)
	queries.Assign(&c.LanguageID, a.ID)
	...
	for _, v := range check {
		if queries.Equal(v.LanguageID, b.LanguageID) {
			bFound = true
		}
		if queries.Equal(v.LanguageID, c.LanguageID) {
			cFound = true
		}
	}
	...
	if err = a.L.LoadNovels(ctx, tx, false, (*[]*Language)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testLanguageToManyAddOpNovels(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if !queries.Equal(a.ID, first.LanguageID) {
			t.Error("foreign key was wrong value", a.ID, first.LanguageID)
		}
		if !queries.Equal(a.ID, second.LanguageID) {
...
func testLanguageToManySetOpNovels(t *testing.T) {
	var err error
	...
	if !queries.IsValuerNil(b.LanguageID) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.LanguageID) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.ID, d.LanguageID) {
		t.Error("foreign key was wrong value", a.ID, d.LanguageID)
	}
	if !queries.Equal(a.ID, e.LanguageID) {
		t.Error("foreign key was wrong value", a.ID, e.LanguageID)
	}
...
func testLanguageToManyRemoveOpNovels(t *testing.T) {
	var err error
