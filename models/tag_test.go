...
func testTagToManyNovels(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	_, err = tx.Exec("insert into \"ste\".\"tag_of_novel\" (\"tag_id\", \"novel_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"ste\".\"tag_of_novel\" (\"tag_id\", \"novel_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := TagSlice{&a}
	if err = a.L.LoadNovels(ctx, tx, false, (*[]*Tag)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTagToManyAddOpNovels(t *testing.T) {
	var err error
...
func testTagToManySetOpNovels(t *testing.T) {
	var err error
...
func testTagToManyRemoveOpNovels(t *testing.T) {
	var err error
