...
func testAuthorToManyNovels(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	_, err = tx.Exec("insert into \"ste\".\"author_of_novel\" (\"author_id\", \"novel_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"ste\".\"author_of_novel\" (\"author_id\", \"novel_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := AuthorSlice{&a}
	if err = a.L.LoadNovels(ctx, tx, false, (*[]*Author)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testAuthorToManyAddOpNovels(t *testing.T) {
	var err error
...
func testAuthorToManySetOpNovels(t *testing.T) {
	var err error
...
func testAuthorToManyRemoveOpNovels(t *testing.T) {
	var err error
...
var (
	authorDBTypes = map[string]string{`ID`: `integer`, `Name`: `text`, `URL`: `text`}
	_             = bytes.MinRead
)
