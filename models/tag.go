...
var (
	tagColumns               = []string{"id", "name", "url"}
	tagColumnsWithoutDefault = []string{"id", "name", "url"}
...
func (o *Tag) Novels(mods ...qm.QueryMod) novelQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"ste\".\"tag_of_novel\" on \"ste\".\"novel\".\"id\" = \"ste\".\"tag_of_novel\".\"novel_id\""),
		qm.Where("\"ste\".\"tag_of_novel\".\"tag_id\"=?", o.ID),
	)
...
func (tagL) LoadNovels(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTag interface{}, mods queries.Applicator) error {
	var slice []*Tag
	var object *Tag
	...
	query := NewQuery(
		qm.Select("\"ste\".\"novel\".*, \"a\".\"tag_id\""),
		qm.From("\"ste\".\"novel\""),
		qm.InnerJoin("\"ste\".\"tag_of_novel\" as \"a\" on \"ste\".\"novel\".\"id\" = \"a\".\"novel_id\""),
...
func (o *Tag) AddNovels(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Novel) error {
	var err error
	...
	for _, rel := range related {
		query := "insert into \"ste\".\"tag_of_novel\" (\"tag_id\", \"novel_id\") values ($1, $2)"
...
// Sets o.R.Tags's Novels accordingly.
// Replaces o.R.Novels with related.
// Sets related.R.Tags's Novels accordingly.
func (o *Tag) SetNovels(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Novel) error {
	query := "delete from \"ste\".\"tag_of_novel\" where \"tag_id\" = $1"
	...
	removeNovelsFromTagsSlice(o, related)
	if o.R != nil {
		o.R.Novels = nil
	}
...
// Sets related.R.Tags.
func (o *Tag) RemoveNovels(ctx context.Context, exec boil.ContextExecutor, related ...*Novel) error {
	var err error
	query := fmt.Sprintf(
		"delete from \"ste\".\"tag_of_novel\" where \"tag_id\" = $1 and \"novel_id\" in (%s)",
...
func removeNovelsFromTagsSlice(o *Tag, related []*Novel) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
...
func Tags(mods ...qm.QueryMod) tagQuery {
	mods = append(mods, qm.From("\"ste\".\"tag\""))
...
func FindTag(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Tag, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"ste\".\"tag\" where \"id\"=$1", sel,
	)
...
func (o *Tag) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"ste\".\"tag\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"ste\".\"tag\" %sDEFAULT VALUES%s"
		}
...
func (o *Tag) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"ste\".\"tag\" SET %s WHERE %s",
...
func (o TagSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"ste\".\"tag\" SET %s WHERE %s",
...
func (o *Tag) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"ste\".\"tag\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *Tag) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"ste\".\"tag\" WHERE \"id\"=$1"
...
func (o *TagSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"ste\".\"tag\".* FROM \"ste\".\"tag\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, tagPrimaryKeyColumns, len(*o))
...
func TagExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"ste\".\"tag\" where \"id\"=$1 limit 1)"
