...
var (
	authorColumns               = []string{"id", "name", "url"}
	authorColumnsWithoutDefault = []string{"name", "url"}
...
func (o *Author) Novels(mods ...qm.QueryMod) novelQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"ste\".\"author_of_novel\" on \"ste\".\"novel\".\"id\" = \"ste\".\"author_of_novel\".\"novel_id\""),
		qm.Where("\"ste\".\"author_of_novel\".\"author_id\"=?", o.ID),
	)
...
func (authorL) LoadNovels(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAuthor interface{}, mods queries.Applicator) error {
	var slice []*Author
	var object *Author
	...
	query := NewQuery(
		qm.Select("\"ste\".\"novel\".*, \"a\".\"author_id\""),
		qm.From("\"ste\".\"novel\""),
		qm.InnerJoin("\"ste\".\"author_of_novel\" as \"a\" on \"ste\".\"novel\".\"id\" = \"a\".\"novel_id\""),
...
func (o *Author) AddNovels(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Novel) error {
	var err error
	...
	for _, rel := range related {
		query := "insert into \"ste\".\"author_of_novel\" (\"author_id\", \"novel_id\") values ($1, $2)"
...
// Sets o.R.Authors's Novels accordingly.
// Replaces o.R.Novels with related.
// Sets related.R.Authors's Novels accordingly.
func (o *Author) SetNovels(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Novel) error {
	query := "delete from \"ste\".\"author_of_novel\" where \"author_id\" = $1"
	...
	removeNovelsFromAuthorsSlice(o, related)
	if o.R != nil {
		o.R.Novels = nil
	}
...
func (o *Author) RemoveNovels(ctx context.Context, exec boil.ContextExecutor, related ...*Novel) error {
	var err error
	query := fmt.Sprintf(
		"delete from \"ste\".\"author_of_novel\" where \"author_id\" = $1 and \"novel_id\" in (%s)",
...
func removeNovelsFromAuthorsSlice(o *Author, related []*Novel) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
...
func Authors(mods ...qm.QueryMod) authorQuery {
	mods = append(mods, qm.From("\"ste\".\"author\""))
...
func FindAuthor(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Author, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"ste\".\"author\" where \"id\"=$1", sel,
	)
...
func (o *Author) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"ste\".\"author\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"ste\".\"author\" %sDEFAULT VALUES%s"
		}
...
func (o *Author) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"ste\".\"author\" SET %s WHERE %s",
...
func (o AuthorSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"ste\".\"author\" SET %s WHERE %s",
...
func (o *Author) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"ste\".\"author\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *Author) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"ste\".\"author\" WHERE \"id\"=$1"
...
func (o AuthorSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"ste\".\"author\" WHERE " +
...
func (o *AuthorSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"ste\".\"author\".* FROM \"ste\".\"author\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, authorPrimaryKeyColumns, len(*o))
...
func AuthorExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"ste\".\"author\" where \"id\"=$1 limit 1)"
