...
func testGenreToManyNovels(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	_, err = tx.Exec("insert into \"ste\".\"genre_of_novel\" (\"genre_id\", \"novel_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"ste\".\"genre_of_novel\" (\"genre_id\", \"novel_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := GenreSlice{&a}
	if err = a.L.LoadNovels(ctx, tx, false, (*[]*Genre)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testGenreToManyAddOpNovels(t *testing.T) {
	var err error
...
func testGenreToManySetOpNovels(t *testing.T) {
	var err error
...
func testGenreToManyRemoveOpNovels(t *testing.T) {
	var err error
...
func testGenresUpdate(t *testing.T) {
	...
	if len(genreColumns) == len(genrePrimaryKeyColumns) {
...
func testGenresSliceUpdateAll(t *testing.T) {
	...
	var fields []string
	if strmangle.StringSliceMatch(genreColumns, genrePrimaryKeyColumns) {
		fields = genreColumns
	} else {
